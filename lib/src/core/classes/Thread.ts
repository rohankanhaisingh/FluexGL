...
export class Thread {
    ...
    public Start(): Thread {
        ...
        if (this.isActive)
            throw new Error("Could not start thread loop because it is already active. Call method Stop() before starting again.");
    ...
    public Stop(): Thread {
        ...
        if (!this.isActive)
            throw new Error("Could not stop the thread loop because it is already inactive. Call method Start() before stopping again.");
    ...
    public SetSimulationUpdateRate(rate: number): Thread {
        ...
        if (!Number.isFinite(rate) || rate <= 0)
            throw new Error(`SetSimulationUpdateRate: invalid rate "${rate}".`);
    ...
    public SetMaxDeltaMs(ms: number): Thread {
        ...
        if (!Number.isFinite(ms) || ms <= 0)
            throw new Error(`SetMaxDeltaMs: invalid value "${ms}".`);
